...
<main class="adam-experience" id="main">
	<header class="<?php echo esc_attr( Public_Hero::root( 'adam-experience-hero', 'archive' ) ); ?>"><div class="<?php echo esc_attr( Public_Hero::element( 'content', 'adam-experience-container' ) ); ?>"><span class="<?php echo esc_attr( Public_Hero::element( 'kicker' ) ); ?>"><?php esc_html_e( 'Região', 'adam-comunidade' ); ?></span><h1 class="<?php echo esc_attr( Public_Hero::element( 'title' ) ); ?>"><?php echo esc_html( $district ); ?></h1><p class="<?php echo esc_attr( Public_Hero::element( 'subtitle' ) ); ?>"><?php echo esc_html( sprintf( __( 'A comunidade ADAM em %s.', 'adam-comunidade' ), $district ) ); ?></p></div></header>
	<div class="adam-experience-container">
		...
		<section class="adam-advanced-map" data-adam-advanced-map data-markers="<?php echo esc_attr( wp_json_encode( $markers ) ); ?>"><div class="adam-section-title"><h2><?php esc_html_e( 'Mapa da região', 'adam-comunidade' ); ?></h2></div><div class="adam-map-layout"><div class="adam-map-canvas" data-adam-map-canvas role="region" aria-label="<?php esc_attr_e( 'Mapa da região', 'adam-comunidade' ); ?>"></div><div class="adam-map-results" data-adam-map-results></div></div></section>
		<?php echo $section( __( 'Equipas', 'adam-comunidade' ), array_map( array( Team_View::class, 'card' ), $teams ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
		<?php echo $section( __( 'Equipas a recrutar', 'adam-comunidade' ), array_map( array( Team_View::class, 'card' ), $recruiting_teams ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
		<?php echo $section( __( 'Campos', 'adam-comunidade' ), array_map( static fn( object $item ): string => Field_View::card( $item, $fields_repo ), $fields ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
		<?php echo $section( __( 'Campos em destaque', 'adam-comunidade' ), array_map( static fn( object $item ): string => Field_View::card( $item, $fields_repo ), $featured_fields ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
