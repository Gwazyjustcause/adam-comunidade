...
$assert( str_contains( $news, 'has_post_thumbnail( $post_id )' ), 'Archive image helper must preserve the native featured image path.' );
$assert( str_contains( $news, 'parse_blocks' ) && str_contains( $news, "'core/image'" ), 'Archive image detection must inspect parsed content blocks.' );
$assert( str_contains( $news, 'starts_with_archive_image' ) && str_contains( $news, "return array( 'html' => '', 'has_image' => false );" ), 'Archive image helper must support the image-first and text-first fallback cases.' );
$assert( ! str_contains( $archive, 'get_media_embedded_in_content' ), 'Archive must not scan all content for a later image.' );
