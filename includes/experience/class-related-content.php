...
final class Related_Content {
	...
	public function for_team( object $team ): void {
		...
		echo $this->section( __( 'Recommended Fields', 'adam-comunidade' ), array_map( fn( object $item ): string => Field_View::card( $item, $this->fields ), $fields ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		echo $this->section( __( 'Partner Discounts', 'adam-comunidade' ), array_map( array( Directory_View::class, 'card' ), array_filter( $partners, static fn( object $item ): bool => (bool) $item->benefits ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		echo $this->section( __( 'Nearby Teams', 'adam-comunidade' ), array_map( array( Team_View::class, 'card' ), $nearby_teams ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
	...
	public function for_field( object $field ): void {
		...
		echo $this->section( __( 'Nearby Fields', 'adam-comunidade' ), array_map( fn( object $item ): string => Field_View::card( $item, $this->fields ), $nearby_fields ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		echo $this->section( __( 'Nearby Partners', 'adam-comunidade' ), array_map( array( Directory_View::class, 'card' ), $partners ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		echo $this->section( __( 'Nearby Shops', 'adam-comunidade' ), array_map( array( Directory_View::class, 'card' ), $shops ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	...
	private function related_news( string $type, int $id ): string {
		...
		return $this->section( __( 'Relevant News', 'adam-comunidade' ), $cards );
	}
