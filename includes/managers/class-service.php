...
final class Service {
	...
	public function invite( string $email, string $entity_type, int $entity_id ): string|\WP_Error {
		...
		$wpdb->query(
			$wpdb->prepare(
				'UPDATE ' . Schema::invitations_table() . ' SET used_at = %s WHERE manager_id = %d AND entity_type = %s AND entity_id = %d AND used_at IS NULL',
				$now,
				$manager_id,
				$entity_type,
				$entity_id
	...
	public function activate( string $token, string $password ): int|\WP_Error {
		...
		$invitation = $wpdb->get_row(
			$wpdb->prepare(
				'SELECT * FROM ' . Schema::invitations_table() . ' WHERE token_hash = %s AND used_at IS NULL AND expires_at > %s',
				hash( 'sha256', $token ),
	...
	private function allowed_fields( string $type ): array {
		...
		return 'team' === $type
			? array_merge( $common, array( 'short_name', 'logo_id', 'gallery', 'team_colour', 'discord', 'youtube', 'tiktok', 'founded', 'members', 'recruitment_status', 'recruitment_min_age', 'recruitment_experience', 'recruitment_equipment', 'recruitment_training', 'equipment_tags' ) )
			: array_merge( $common, array( 'availability', 'rules', 'opening_hours', 'max_players', 'min_players', 'recommended_players' ) );
	}
