	...
	private function edit_form(): void {
		...
		<form class="adam-card adam-manager-form" data-adam-manager-editor method="post" enctype="multipart/form-data" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
			...
			<div class="adam-manager-media">
				...
				<?php if ( 'field' !== $type && ! empty( $record->logo_id ) && wp_attachment_is_image( (int) $record->logo_id ) ) : ?><div class="adam-manager-current-image adam-manager-current-image--logo"><strong><?php esc_html_e( 'Logótipo incluído na proposta', 'adam-comunidade' ); ?></strong><?php echo Team_View::logo( $record, array( 'class' => 'adam-manager-current-image__logo' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?><label><input type="checkbox" name="remove_logo" value="1"> <?php esc_html_e( 'Propor remoção do logótipo', 'adam-comunidade' ); ?></label></div><?php endif; ?>
				<?php if ( 'field' !== $type ) : ?><label><?php esc_html_e( 'Novo logótipo (opcional)', 'adam-comunidade' ); ?></label><?php Upload_Component::render( array( 'mode' => 'file', 'kind' => 'image', 'name' => 'manager_logo', 'class' => 'adam-upload--team-logo', 'accept' => $image_accept, 'max_size_mb' => $image_policy['max_size_mb'] ) ); ?><?php endif; ?>
