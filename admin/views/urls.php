...
<div class="wrap adam-comunidade-admin">
	<header class="adam-page-header">
		<div>
			<h1><?php esc_html_e( 'Community URLs', 'adam-comunidade' ); ?></h1>
			<p><?php esc_html_e( 'Routing is controlled by the WordPress pages below. Titles and content are edited in the normal page editor.', 'adam-comunidade' ); ?></p>
		</div>
	</header>
	<form class="adam-card" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="post">
		...
		<table class="widefat striped">
			<thead>
				<tr>
					<th><?php esc_html_e( 'Module', 'adam-comunidade' ); ?></th>
					<th><?php esc_html_e( 'Current Page', 'adam-comunidade' ); ?></th>
					<th><?php esc_html_e( 'Current URL', 'adam-comunidade' ); ?></th>
					<th><?php esc_html_e( 'Slug', 'adam-comunidade' ); ?></th>
					<th><?php esc_html_e( 'Preview', 'adam-comunidade' ); ?></th>
					<th><?php esc_html_e( 'Page', 'adam-comunidade' ); ?></th>
				</tr>
			</thead>
			<tbody>
					...
					<tr>
						...
						<td>
							<select name="page_ids[<?php echo esc_attr( $module ); ?>]">
								<option value="0"><?php esc_html_e( 'Select a page', 'adam-comunidade' ); ?></option>
								<?php foreach ( $available_pages as $available_page ) : ?>
									<option value="<?php echo esc_attr( $available_page->ID ); ?>" <?php selected( $row['id'], $available_page->ID ); ?>><?php echo esc_html( $available_page->post_title ?: sprintf( __( 'Page #%d', 'adam-comunidade' ), $available_page->ID ) ); ?></option>
								<?php endforeach; ?>
							</select>
						</td>
						...
						<td>
							<?php if ( $page ) : ?>
								<a class="button" href="<?php echo esc_url( get_edit_post_link( $row['id'], 'raw' ) ); ?>"><?php esc_html_e( 'Edit Page', 'adam-comunidade' ); ?></a>
							<?php endif; ?>
						</td>
					</tr>
				<?php endforeach; ?>
		...
		<?php submit_button( __( 'Save URL Settings', 'adam-comunidade' ) ); ?>
	</form>
</div>
