...
<div class="wrap adam-comunidade-admin adam-directory-editor">
	...
	<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
		...
		<section class="adam-comunidade__card adam-directory-panel" data-adam-panel="details" hidden>
			...
			<?php if ( 'partner' === $type ) : ?><h3><?php esc_html_e( 'Informação de marca (opcional)', 'adam-comunidade' ); ?></h3><p class="description"><?php esc_html_e( 'Utilize estes campos quando a categoria selecionada for Marca.', 'adam-comunidade' ); ?></p><div class="adam-directory-grid"><label><?php esc_html_e( 'País', 'adam-comunidade' ); ?><input type="text" name="entry[country]" value="<?php echo esc_attr( $value( 'country' ) ); ?>"></label><label><input type="checkbox" name="entry[official_distributor]" value="1" <?php checked( $value( 'official_distributor' ) ); ?>> <?php esc_html_e( 'Distribuidor oficial', 'adam-comunidade' ); ?></label></div><label><?php esc_html_e( 'Produtos populares', 'adam-comunidade' ); ?><textarea name="entry[popular_products]" rows="6"><?php echo esc_textarea( $value( 'popular_products' ) ); ?></textarea></label><?php endif; ?>
		...
		<section class="adam-comunidade__card adam-directory-panel" data-adam-panel="relationships" hidden>
			...
			<?php foreach ( array( 'partner' => __( 'Parceiros relacionados', 'adam-comunidade' ), 'team' => __( 'Equipas associadas / patrocinadas', 'adam-comunidade' ), 'field' => __( 'Campos associados / apoiados', 'adam-comunidade' ) ) as $target_type => $label ) : ?>
				<?php if ( 'partner' !== $type && 'partner' === $target_type ) { continue; } ?>
